            ...
            {coins?.length ?
                coins.map((el, i) => (
                    <Link key={i} to={`/coin/${el.stockNumber}`}>
                        <img className={`${stockNumber === el.stockNumber ? 'current' : ''}`} loading='lazy' src={el.idPhoto ? `${Api.url2}${el.stockNumber.toString()[0]}0000/${el.stockNumber}q00Trumb100x50.jpg` : '/images/noimage.jpeg'} alt="" />
                    </Link>
                ))
            :<></>}
        </div>
